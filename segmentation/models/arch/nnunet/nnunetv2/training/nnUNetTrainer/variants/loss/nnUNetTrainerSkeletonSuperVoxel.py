...
from segmentation.models.arch.nnunet.nnunetv2.training.loss.supervoxel_loss_multiclass import SuperVoxelLoss
...
class nnUNetTrainerSkeletonSuperVoxel(nnUNetTrainer):
    ...
    def _build_loss(self):
        ...
        loss = DC_SkelREC_and_CE_loss(soft_dice_kwargs={'batch_dice': self.configuration_manager.batch_dice, 
                                      ...
                                      soft_skelrec_kwargs={'batch_dice': self.configuration_manager.batch_dice, 
                                                           'smooth': 1e-5, 'do_bg': False, 'ddp': self.is_ddp}, 
                                      ce_kwargs={'num_classes': 3}, weight_ce=1, weight_dice=1, weight_srec=self.weight_srec, 
                                      ignore_label=self.label_manager.ignore_label, dice_class=MemoryEfficientSoftDiceLoss, ce_class=SuperVoxelLoss)
...
class nnUNetTrainerMedialSurfaceSuperVoxel(nnUNetTrainerSkeletonSuperVoxel):
    @staticmethod
    def get_training_transforms(
            patch_size: Union[np.ndarray, Tuple[int]],
            rotation_for_DA: RandomScalar,
